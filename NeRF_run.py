...
def create_nerf(args):
    ...
    """
    embed_fn, input_ch, embed_obj = get_embedder(args.multires, device, args.i_embed)
    embed_obj = embed_obj.to(device)
    embed_fn = lambda x, eo=embed_obj: eo(x)
    grad_vars = list(embed_obj.parameters())

    input_ch_views = 0
    embeddirs_fn = None
    embedviews_obj = None
    if args.use_viewdirs:
        embeddirs_fn, input_ch_views, embedviews_obj = get_embedder(args.multires_views, device, args.i_embed)
        embedviews_obj = embedviews_obj.to(device)
        embeddirs_fn = lambda x, eo=embedviews_obj: eo(x)
        grad_vars += list(embedviews_obj.parameters())
    ...
    return render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, embed_obj
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    """
    # act_fn: post-activation function???
    raw2alpha = lambda raw, dists, act_fn=F.relu: 1. - torch.exp(-act_fn(raw) * dists)
    # dists: [s2-s1, ..., sn-s_n-1, 1e10]
...
def render_rays(ray_batch,
    ...
    Args:
      ...
      legendre_integrate: bool. If True, use legendre integration in small regions.
...
def train():
    ...
    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, embed_obj = create_nerf(args)
    global_step = start
    ...
    for i in trange(start, N_iters):
        ...
        if i % args.i_weights == 0:
            ...
            torch.save({
                ...
                'embedder_fn_state_dict': embed_obj.state_dict(),
